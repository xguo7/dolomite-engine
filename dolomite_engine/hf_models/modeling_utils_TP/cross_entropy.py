...
class TensorParallelCrossEntropy(torch.autograd.Function):
    @staticmethod
    def forward(ctx, lm_logits: torch.Tensor, labels: torch.Tensor, vocab_size: int) -> torch.Tensor:
        tp_group = ProcessGroupManager.get_tensor_parallel_group()

        # Maximum value along vocab dimension across all GPUs.
        logits_max = torch.max(lm_logits, dim=-1)[0]
        ...
        exp_logits = torch.exp(lm_logits)
